...
pub fn build_mp4(config: &Mp4TrackConfig, samples: &[Mp4Sample]) -> Vec<u8> {
    ...
    let mdhd_size = 8 + 4 + 4 + 4 + 4 + 4 + 4; // fullbox(8+4) + creation(4) + mod(4) + ts(4) + dur(4) + lang+pad(4)
    let mdia_size = 8 + mdhd_size + hdlr_size + minf_size;
    let tkhd_size = 8 + 4 + 4 + 4 + 4 + 4 + 4 + 8 + 2 + 2 + 2 + 2 + 36 + 4 + 4;
    let trak_size = 8 + tkhd_size + mdia_size;
    let mvhd_size = 8 + 4 + 4 + 4 + 4 + 4 + 4 + 2 + 10 + 36 + 24 + 4;
    ...
    write_box(&mut buf, b"moov", |b| {
        ...
        write_box(b, b"trak", |b| {
            // tkhd (version 0, flags = track_enabled | track_in_movie)
            write_full_box(b, b"tkhd", 0, 3, |b| {
                ...
                write_identity_matrix(b);
                b.extend_from_slice(&((config.width as u32) << 16).to_be_bytes()); // width 16.16
                b.extend_from_slice(&((config.height as u32) << 16).to_be_bytes()); // height 16.16
            });
