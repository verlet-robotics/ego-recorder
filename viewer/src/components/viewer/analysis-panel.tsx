...
import type { AnalysisResult, Verdict, EgorecListItem } from "@/lib/types";
...
export function AnalysisPanel({ fileName }: AnalysisPanelProps) {
  ...
  const handlePrune = async () => {
    ...
    try {
      const res = await fetch(
        `/api/files/${encodeURIComponent(fileName)}/prune`,
        { method: "POST" },
      );
      const data = await res.json();
      if (!res.ok) {
        setActionError(data.error || "Prune failed");
        return;
      }
      removeFile(fileName);
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Prune failed");
    } finally {
  ...
  const handleSplice = async (replaceOriginal: boolean) => {
    ...
    try {
      const res = await fetch(
        `/api/files/${encodeURIComponent(fileName)}/splice`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ replaceOriginal }),
        },
      );
      const data = await res.json();
      if (!res.ok) {
        setActionError(data.error || "Splice failed");
        return;
      }
      // Add new segment files to the store
      ...
      setActionError(err instanceof Error ? err.message : "Splice failed");
    } finally {
...
export function PrunedFilesPanel() {
  ...
  const loadPruned = async () => {
    try {
      const res = await fetch("/api/pruned");
      const data = await res.json();
      setPrunedFiles(data.files || []);
      setLoaded(true);
    } catch {
      setPrunedFiles([]);
      setLoaded(true);
